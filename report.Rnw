...
@
\section{Methods}
The association between potential risk factors and the rate of rotaviral diarrhoea was examined first by calculating rate ratios and testing the null hypothesis that these were not equal to one using \textit{z}-tests. \\
\indent Interactions between variables were examined first by pairwise Mantel-Haenszel analyses where interactions were considered important if they had a substantial effect on the rate ratio.
Interactions considered important in Mantel-Haenszel analyses were then examined by inclusion or exclusion in multivariable models and were considered important if they had a substantial effect on the hazard ratio.\\
\indent To adjust for the effect of calendar time and age, data were split on these variables. 
Since the first few months of age were considered most likely to show the greatest variation in the rate of rotaviral diarrhoea, age was split into bands of increasing width. 
...
Calendar time was therefore split on annual quarters, starting from December.
...
Both being of a low birth weight and history of neonatal rotaviral diarrhoea could reflect a general ill health and predispose to rotaviral disease later. 
Mother's education level and a family's socio-economic status (SES) could reflect a number of potential aspects about the environment in which subjects lived and a lower SES was presumed to increase the risk of rotaviral diarrhoea. 
...
Animal ownership was expected to increase the risk of  disease by decreasing general levels of hygiene, and perhaps by zoonotic infections making rotavirus infection more likely. 
...
To account for the effect of repeated observations on an individual, a random effects model was fitted. \\
...
\indent Univariable analyses were performed in \Sexpr{substring(R.version.string,1,16)}, Mantel-Haenszel stratified analyses and multivariable poisson regression which were performed in Stata version 12.\\
%
\section{Results}
...
\indent By univariable analysis there was moderate evidence that the rate of rotaviral diarrhoea was lower in December 2002 to February 2003 and March to May 2003 compared to the September to November 2003 (table \ref{trend}).
Following adjustment for subject age, there was strong evidence that the rate of rotaviral diarrhoea was different from the baseline for a number of periods, and that the fit of the multivariable model was better than the fit of the univariable model. \\
...
Finally, after adjusting for other factors, there was no evidence that the rate of rotaviral diarrhoea was either higher or lower in children who had pervious episodes compared to those who had not(table \ref{multivar}, p = 0.58 lr test of model 2 vs model 3). \\
%
\section{Discussion}
\indent In this study rotaviral diarrhoea was a prevalent disease, affecting over 35 \% of children in the study. 
After adjustment for other factors, being of male sex and living in a household where beedi were made were were associated with higher rates of disease.  
...
\indent One limitation of these results is that due to software limitations, it was not possible to study interactions in multivariable models. 
This inability is likely to have concealed some important combinations of conditions, such as being of low birth weight and having neonatal rotaviral diarrhoea, which might have been associated with increased rates of diease. \\
...
Had those children who were not enrolled had experienced either higher or lower rates of disease and different exposures, the associations determined here would not be reliable. 
...
The nature of this testing is not known, but errors in testing could affect the strength of relationships determined here.
Random error in diagnosis resulting in under diagnosis would bias any associations towards the null, potentially leading to type II error.
Non-random error in diagnosis could result in over or underestimation of associations.\\
\indent Unmeasured confounding could account for some associations observed in this study. 
The reason for the association between male sex and rotaviral diarrhoea is not clear and if another co-mobidity which predisposed to rotaviral diarrhoea were more prevalent among male infants, then it could account for the difference. 
...
\toprule
 ...
 &  & HR (95 \% CI) & \shortstack{Wald \\p-value} & HR (95 \% CI) & \shortstack{Wald \\p-value} & HR (95 \% CI) & \shortstack{Wald \\p-value} \\ 
 \midrule
...
Model 1: Poisson regression with random effects, adjusted for age and calendar time, not adjusted for number of previous episodes or any previous episode, p for model < 0.001. 
...
Model 3: Poisson regression with random effects, adjusted for age and calendar time, and any previous episode, p for model < 0.001.}
\label{multivar}
\end{sidewaystable}
