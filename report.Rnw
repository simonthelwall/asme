...
srOrWrapper <- function(x){
  cbind(HR=round(1/exp(coef(x)), 2),
        se=round(summary(m1)$table[,2], 2), 
        lci=round((1/exp(coef(x)))/exp(1.96*summary(m1)$table[,2]), 2),
        uci=round((1/exp(coef(x)))*exp(1.96*summary(m1)$table[,2]), 2),
        p=round(summary(m1)$table[,4], 4))
}
